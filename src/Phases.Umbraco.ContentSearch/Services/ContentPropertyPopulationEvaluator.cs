...
/// </summary>
internal static partial class ContentPropertyPopulationEvaluator
{
        ...
        string? culture = null)
    {
        ...
        if (property.PropertyType.VariesByCulture())
        {
            ...
            foreach (var propertyCulture in property.Values
                         ...
                         .Where(static cultureCode => !string.IsNullOrWhiteSpace(cultureCode))
    ...
    private static bool HasPopulatedStringValue(string rawValue, string? propertyEditorAlias)
    {
        ...
        if (IsRichTextEditor(propertyEditorAlias))
        {
            return HasPopulatedRichTextValue(trimmed);
        }
    ...
    private static bool IsRichTextEditor(string? propertyEditorAlias)
        => propertyEditorAlias is not null
            && (propertyEditorAlias.Equals("Umbraco.TinyMCE", StringComparison.OrdinalIgnoreCase)
                || propertyEditorAlias.Equals("Umbraco.RichText", StringComparison.OrdinalIgnoreCase));

    private static bool HasPopulatedRichTextValue(string value)
    {
